...
window.app.components.carousel = async () => {
    ...
    try {
        ...
        // --- DIAGNOSTIC ERROR UI ---
        // If the proxy fails (e.g., returns 404 HTML instead of JSON), this will trigger.
        if (!response || !response.data || response.data.length === 0) {
            container.innerHTML = `
                <div class="w-full aspect-video flex flex-col items-center justify-center bg-red-950/20 border-b border-red-500/50 p-6">
                    <i class="fas fa-exclamation-triangle text-red-500 text-4xl mb-3"></i>
                    <h2 class="text-red-500 font-black text-xl mb-1">API CONNECTION FAILED</h2>
                    <p class="text-gray-400 text-xs text-center max-w-md">
                        The carousel could not fetch data from <b>/api/anikoto</b>.<br><br>
                        <b>If you are testing locally:</b> Proxies do not work on local files. You must test on your live Cloudflare URL.<br>
                        <b>If you are on Cloudflare:</b> Ensure your <code>_redirects</code> file is in the root directory and contains no typos.
                    </p>
                </div>
            `;
            return;
        ...
        // Grab top 5 items and stash them safely in state
                ...
                <div class="carousel-slide absolute inset-0 transition-opacity duration-1000 ease-in-out ${i === 0 ? 'opacity-100 z-20' : 'opacity-0 z-10'}" id="slide-${i}">
                    ...
                    <div class="absolute bottom-8 left-4 md:bottom-12 md:left-12 z-30 max-w-[85%] md:max-w-2xl pr-8">
                        ...
                        <div class="flex gap-3">
                            <button onclick="window.location.hash='watch/null/1/sub'" class="bg-[#F47521] text-black px-6 py-2.5 rounded shadow-[0_0_15px_rgba(244,117,33,0.3)] font-bold text-xs uppercase tracking-wider hover:bg-white hover:shadow-none transition-all flex items-center gap-2">
