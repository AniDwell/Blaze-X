...
window.app.components.info = async () => {
    ...
    try {
        // 1. Fetch from your Anikoto Proxy using standard fetch
        ...
        const baseAnime = payload.anime || payload; // Title, backdrop, details object
        const episodesList = payload.episodes || baseAnime.episodes || []; // Episode array

        if (!baseAnime || (!baseAnime.title && !baseAnime.name)) throw new Error("Invalid anime data received.");

        // 2. Fetch Exhaustive Sync Data from AniList for deep details & trailer
        let aniData = {};
        ...
        // 3. Construct Global Page State Object
        ...
        // Initialize internal state for UI controls
        window.app.state.activeInfoTab = 'episodes'; // default tab
        window.app.state.epSearchValue = '';
        window.app.state.epRangeFilter = '1-100';

        // 4. Smart "Play" or "Resume" Button Logic
        ...
        // 5. Render the page shell and default content
...
function renderAnimeInfoShell() {
    ...
    // Clean up HTML tags from AniList synopsis data
    const cleanDesc = data.synopsis.replace(/<[^>]*>?/gm, '');

    // Format genres string
    const genresStr = ani.genres ? ani.genres.join(' • ') : 'genres';

    container.innerHTML = `
        <div class="relative w-full h-[65vh] md:h-[75vh] bg-black overflow-hidden mt-[60px] md:mt-0">
            
            <div id="info-hero-bg" class="absolute inset-0 z-0 transition-opacity duration-1000">
                <img src="${data.banner}" class="w-full h-full object-cover opacity-60">
            </div>
            ...
            <div class="absolute inset-0 z-20 flex flex-col md:flex-row items-end md:items-center px-4 pb-8 md:px-12 gap-6 md:gap-10">
                ...
                <div class="w-[30%] md:w-1/4 max-w-[220px] md:max-w-[280px] flex-shrink-0 rounded-lg overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.8)] border-2 border-white/10 hidden md:block z-30">
                    <img src="${data.poster}" class="w-full h-full object-cover">
                </div>

                <div class="flex-1 w-full max-w-4xl z-30">
                    <h1 class="text-3xl md:text-5xl lg:text-6xl font-black text-white mb-2 drop-shadow-[0_4px_8px_rgba(0,0,0,0.8)] leading-tight tracking-tight">${data.title}</h1>
                    <p class="text-sm md:text-base text-gray-400 font-medium mb-1 drop-shadow-md pr-12">${data.jpTitle}</p>
                    <p class="text-[10px] md:text-xs text-gray-500 font-medium tracking-wide mb-6 uppercase">${genresStr}</p>
                    
                    <div class="flex flex-wrap gap-3 mb-6">
                        <button onclick="if('${data.smartPlayAction}' !== '') { window.location.href='play.html?id=${data.smartPlayAction}&anime=${data.id}' } else { alert('No episodes!') }" class="bg-[#F47521] text-black px-8 py-3.5 rounded-lg shadow-[0_0_15px_rgba(244,117,33,0.3)] font-black text-xs md:text-sm uppercase tracking-wider hover:bg-white transition-all flex items-center gap-2">
        ...
        <div class="w-full bg-[#050505] min-h-[50vh]">
            <div class="flex items-center gap-8 px-4 md:px-12 border-b border-white/10 text-xs md:text-sm font-bold uppercase tracking-widest pt-4">
                ...
                <button onclick="window.app.switchInfoTab('info')" id="tab-info" class="pb-3 transition-colors ${window.app.state.activeInfoTab === 'info' ? 'text-white border-b-2 border-[#F47521]' : 'text-gray-500 hover:text-white'}">Information</button>
            </div>
    ...
    // Setup Trailer Timer (mute=0 for audio on, controls=0 hides youtube UI, pointer-events-none prevents direct interaction)
    if (data.trailerId) {
        setTimeout(() => {
            ...
            if (trailerContainer && heroBg) {
                ...
                `;
                // Wait 1.5 seconds for video to buffer slightly before fading backdrop away
                setTimeout(() => {
...
window.app.switchInfoTab = (tabName) => {
    ...
    // Update Menu Styling (Orange underline for active)
...
function renderDynamicTabContent() {
    ...
    if (window.app.state.activeInfoTab === 'info') {
        // --- INFORMATION TAB: Deep Sync with AniList for staff/studios/titles ---
        const ani = data.aniList;
        ...
        const genresHtml = ani.genres ? ani.genres.map(g => `<span class="bg-white/10 px-3 py-1 rounded-full text-xs text-white font-medium">${g}</span>`).join('') : 'N/A';
        const studios = ani.studios?.nodes?.map(s => s.name).join(', ') || 'Unknown';
        const staffHtml = ani.staff?.nodes?.map(s => `<div class="bg-[#111] p-3 rounded-lg border border-white/5 shadow-inner"><div class="font-bold text-white text-sm">${s.name.full}</div><div class="text-xs text-gray-500">${s.primaryOccupations.join(', ')}</div></div>`).join('') || 'Unknown';

        contentArea.innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="md:col-span-1 flex flex-col gap-6 bg-[#0a0a0a] p-6 rounded-xl border border-white/5 shadow-lg">
                    <div><span class="text-gray-600 text-xs font-bold uppercase block mb-1 tracking-wider">Score</span><span class="text-[#F47521] font-black text-2xl"><i class="fas fa-star text-base"></i> ${ani.averageScore || '?'}% Average</span></div>
                    <div><span class="text-gray-600 text-xs font-bold uppercase block mb-1 tracking-wider">Status</span><span class="text-white font-medium capitalize">${ani.status ? ani.status.toLowerCase().replace('_', ' ') : 'Unknown'}</span></div>
                    <div><span class="text-gray-600 text-xs font-bold uppercase block mb-1 tracking-wider">Format</span><span class="text-white font-medium">${ani.format || 'TV'}</span></div>
                    <div><span class="text-gray-600 text-xs font-bold uppercase block mb-1 tracking-wider">Source Material</span><span class="text-white font-medium capitalize">${ani.source ? ani.source.toLowerCase().replace('_', ' ') : 'N/A'}</span></div>
                    <div><span class="text-gray-600 text-xs font-bold uppercase block mb-1 tracking-wider">Main Studio</span><span class="text-white font-medium">${studios}</span></div>
                    
                    ${ani.synonyms && ani.synonyms.length > 0 ? `
                        <div><span class="text-gray-600 text-xs font-bold uppercase block mb-1 tracking-wider">Alternative/Synonyms</span>
                        <div class="text-gray-300 text-xs leading-relaxed font-medium space-y-1">${ani.synonyms.map(t => `<p>• ${t}</p>`).join('')}</div></div>
                    ` : ''}
                </div>

                <div class="md:col-span-2 flex flex-col gap-10">
                    <div>
                        <h3 class="text-white text-lg font-black mb-5 tracking-tight border-b-2 border-[#F47521] inline-block pb-1">Genres</h3>
                        <div class="flex flex-wrap gap-2.5">${genresHtml}</div>
                    </div>
                    
                    <div>
                        <h3 class="text-white text-lg font-black mb-5 tracking-tight border-b-2 border-[#F47521] inline-block pb-1">Primary Staff & Roles</h3>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">${staffHtml}</div>
                    </div>
                </div>
            </div>
        `;
    } else {
        // --- EPISODES TAB: Numeric Tiles with search, filter dropdown, and filler highlighter ---
        const totalEps = data.episodes.length;
        let rangeOptions = '';
        
        if (totalEps > 0) {
            // Calculate numeric group ranges for the dropdown (Groups of 100)
        ...
        contentArea.innerHTML = `
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10 bg-[#0a0a0a] p-4 md:p-5 rounded-xl border border-white/5 shadow-lg">
                
                <div class="relative w-full sm:w-64">
                    <select id="ep-range-dropdown" onchange="window.app.updateEpFilterRange(this.value)" class="appearance-none w-full bg-[#111] border border-white/10 text-white text-sm font-bold py-3 pl-4 pr-10 rounded-lg cursor-pointer outline-none hover:border-white/30 transition-colors focus:border-[#F47521]">
                        ${rangeOptions}
                    </select>
                ...
                <div class="relative w-full sm:w-auto flex-1 max-w-sm">
                    <input type="number" id="episode-search-box" value="${window.app.state.epSearchValue}" onkeyup="window.app.runEpisodeSearch(this.value)" placeholder="Search episode #..." class="w-full bg-[#111] border border-white/10 text-white text-sm py-3 pl-10 pr-4 rounded-lg outline-none focus:border-[#F47521] transition-colors placeholder-gray-600 font-medium">
                    <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-500"></i>
                </div>
            </div>

            <div id="numeric-episodes-grid" class="grid grid-cols-6 sm:grid-cols-10 md:grid-cols-12 lg:grid-cols-16 gap-2 md:gap-3">
                </div>
        `;
...
// --- EPISODE FILTERING & SUB-RENDER ENGINE ---

window.app.updateEpFilterRange = (val) => {
    window.app.state.epRangeFilter = val;
    document.getElementById('episode-search-box').value = ''; // Reset search input
    window.app.state.epSearchValue = '';
    renderNumericEpisodeGrid();
};
...
function renderNumericEpisodeGrid() {
    ...
    if (searchVal !== '') {
        // Run specific search, ignore range dropdown
    ...
    if (episodesToRender.length === 0) {
        gridDiv.innerHTML = `<div class="col-span-full text-center py-10 text-gray-500 flex flex-col items-center"><i class="fas fa-video-slash text-3xl mb-4"></i> No episodes match criteria.</div>`;
        return;
    }

    let gridHtml = '';
    episodesToRender.forEach((ep) => {
        // Calculate dynamic episode number if needed
        const originalArrayIdx = data.episodes.findIndex(e => e.id === ep.id);
        const epNumber = ep.number || (originalArrayIdx + 1);
        
        // --- FILLER HIGHLIGHTER MOCK LOGIC ---
        // Your current API doesn't provide filler data. I am mocking this by flagging episodes containing 'filler' or 'recap' in their title.
        // If your API ever returns `ep.is_filler`, use that standard instead.
        const epTitleLower = (ep.title || '').toLowerCase();
        const isActuallyFiller = epTitleLower.includes('filler') || epTitleLower.includes('recap'); 
        
        const fillerIconDot = isActuallyFiller ? `<div class="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full shadow-[0_0_10px_rgba(239,68,68,1)]" title="Filler/Recap Episode"></div>` : '';
        const hoverClasses = isActuallyFiller ? 'border-red-500/20 text-gray-300 hover:bg-red-500 hover:text-white hover:border-red-500 shadow-red-500/10' : 'border-white/5 text-gray-300 hover:bg-[#F47521] hover:text-white hover:border-[#F47521] shadow-inner';
        ...
        gridHtml += `
            <button onclick="window.location.href='play.html?id=${ep.id}&anime=${data.id}'" class="relative w-full aspect-square flex items-center justify-center rounded border transition-all duration-200 group bg-white/5 ${hoverClasses}">
                <span class="font-bold text-sm md:text-base">${epNumber}</span>
                ${fillerIconDot}
            </button>
        `;
    });
...
window.app.togglePageDesc = () => {
    ...
    if (descP.classList.contains('line-clamp-3') || descP.classList.contains('line-clamp-2')) {
        descP.className = "text-xs md:text-sm text-gray-300 leading-relaxed drop-shadow-md pr-4";
...
window.app.addToLibrary = async (id, title, img) => {
    ...
    if (profile.watchlist && profile.watchlist.some(item => item.id == formattedAnimeEntry.id)) return alert("This series is already in your Library!");
    
    // Add locally to state first for immediate UI
    ...
    try {
        ...
        alert("Added locally, but failed to sync to your account. Check your connection!");
    }
};
                    coverImage { extraLarge } 
                    trailer { id site } 
