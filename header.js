...
window.app.components.header = () => {
    ...
    // Shows user image if logged in, otherwise shows a slick SVG that opens auth
    ...
    container.innerHTML = `
        <header class="fixed top-0 w-full z-[80] bg-black md:bg-transparent md:bg-gradient-to-b md:from-black/90 md:via-black/50 md:to-transparent transition-all duration-300 px-4 py-3 md:px-8 flex justify-between items-center" id="main-header-bar">
    ...
    // Turns the desktop gradient into a solid black bar when scrolling down for readability
    window.addEventListener('scroll', () => {
        const header = document.getElementById('main-header-bar');
        if (header) {
            if (window.scrollY > 50) {
                header.classList.add('bg-black', 'shadow-md');
                header.classList.remove('md:bg-transparent', 'md:bg-gradient-to-b');
            } else {
                header.classList.remove('bg-black', 'shadow-md');
                header.classList.add('md:bg-transparent', 'md:bg-gradient-to-b');
            }
        }
    });
};

// Helper function to generate sidebar buttons
function createSidebarLink(view, label, icon, isActive) {
    ...
    return `
        <button onclick="window.app.toggleSidebar(false); /* Insert routing logic here later */" class="w-full flex items-center gap-4 px-4 py-3 rounded-lg transition-colors group text-left ${activeClass}">
...
// Global toggle logic for the sidebar
