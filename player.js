...
window.app.components.player = async () => {
    ...
    try {
        ...
        const formatTime = (sec) => {
            // Fix: Fallback for NaN or Infinity before duration resolves
        ...
        function buildSettingsMenu(hls, vid) {
            ...
            if (hls && hls.levels) {
                ...
                // Fix: Event delegation instead of window globals
                qList.onclick = (e) => {
    ...
    // --- TEARDOWN & CLEANUP ---
    // Call this method from your router when navigating away: window.app.components.player.destroy()
    window.app.components.player.destroy = () => {
        ...
        // Remove global listeners to prevent memory leaks across routes
