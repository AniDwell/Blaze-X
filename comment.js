...
window.app.initDraggableSheet = () => {
    ...
    let startY = 0; let isDragging = false; let currentHeightVh = 50; 

    const dragStart = (e) => { startY = e.type.includes('mouse') ? e.clientY : e.touches[0].clientY; isDragging = true; panel.style.transition = 'none'; currentHeightVh = window.app.state.commentHeight || 50; };
    ...
    const dragEnd = () => { if (!isDragging) return; isDragging = false; panel.style.transition = 'height 0.3s ease-out'; const currentVh = parseFloat(panel.style.height); if (currentVh > 70) { window.app.state.commentHeight = 90; panel.style.height = `90vh`; } else if (currentVh > 25) { window.app.state.commentHeight = 50; panel.style.height = `50vh`; } else { window.app.closeComments(); } };
...
window.app.loadComments = async (animeId, profile) => {
    ...
    try {
        ...
        window.app.commentsUnsub = firestore.onSnapshot(q, (snapshot) => {
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center gap-4 mt-2">
                                            <button onclick="window.app.toggleCommentReaction('${comment.id}', this)" class="${likeColor} text-[10px] font-bold uppercase tracking-wider transition-colors flex items-center gap-1"><i class="fas fa-thumbs-up"></i> <span>${likesCount}</span></button>
                                            <button onclick="window.app.prepareReply('${comment.id}', '${comment.userName}')" class="text-gray-500 hover:text-white text-[10px] font-bold uppercase tracking-wider transition-colors flex items-center gap-1"><i class="fas fa-reply"></i> Reply</button>
                                        </div>
                                    </div>
                                </div>
                                ${hasReplies ? `<div class="replies-wrapper hidden flex flex-col gap-2 mt-2 w-full">${childRepliesHtml}</div><button onclick="this.previousElementSibling.classList.toggle('hidden'); this.innerHTML = this.previousElementSibling.classList.contains('hidden') ? '<i class=\\'fas fa-chevron-down mr-1\\'></i> Show Replies' : '<i class=\\'fas fa-chevron-up mr-1\\'></i> Hide Replies'" class="text-[9px] font-bold text-[#F47521] mt-2 self-start flex items-center hover:text-white transition-colors"><i class="fas fa-chevron-down mr-1"></i> Show Replies</button>` : ''}
                            </div>
                        `;
                    });
                    return html;
            ...
            container.innerHTML = `<div class="text-center text-red-500 text-xs py-10">Live sync failed.</div>`;
        });
            ...
            if (replyToId) {
                ...
                if (parentComment.exists() && parentComment.data().userId !== profile.uid) {
                    await firestore.addDoc(firestore.collection(window.app.db, `users/${parentComment.data().userId}/notifications`), {
                        type: 'reply', fromUser: profile.name || "A user", animeId, episodeNumber: epNum, timestamp: firestore.serverTimestamp(), read: false
                    });
                }
            }
        }
